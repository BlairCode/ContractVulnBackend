 ...
 constructor( address medianiserAddress, uint256 setTimeBetweenPriceAdjustments ) public payable {
 symbol = "PEG";
 name = "PEG Stablecoin";
 decimals = 18;
 ...
 _totalSupply = feedPrice.mul(address(this).balance).div( 10**uint256(decimals) );
 ...
 function transfer(address to, uint256 tokens) public canTriggerPriceAdjustment override returns (bool success) {
 ...
 function approve(address spender, uint256 tokens) public canTriggerPriceAdjustment override returns (bool success) {
 allowed[msg.sender][spender] = tokens;
 emit Approval(msg.sender, spender, tokens);
 return true;
 }
 function transferFrom(address from, address to, uint256 tokens) public canTriggerPriceAdjustment override returns (bool success) {
 require(to != address(0), "can't send to 0 address, use burn");
 ...
 function allowance(address owner, address spender) public view override returns (uint256 allowancePEG) {
 return allowed[owner][spender];
 }
 function approveAndCall(address spender, uint256 tokens, bytes memory data) public canTriggerPriceAdjustment returns (bool success) {
 allowed[msg.sender][spender] = tokens;
 emit Approval(msg.sender, spender, tokens);
 ...
 feedPrice = feedPrice.mul(address(this).balance).div( 10**uint256(decimals) );
